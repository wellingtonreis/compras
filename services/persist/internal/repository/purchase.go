...
func SavePurchaseItemDocuments(db *mongodb.MongoDB, itemPurchaseMessage *dto.ItemPurchaseMessage) ([]entity.QuotationHistory, error) {
	...
	_, err := collection.UpdateOne(
		context.Background(),
		bson.M{"cotacao": itemPurchaseMessage.Quotation},
		bson.M{"$set": bson.M{"dadosapi": itemPurchaseMessage.Items}},
	...
	pipeline := mongo.Pipeline{
		{{Key: "$match", Value: bson.M{"cotacao": itemPurchaseMessage.Quotation}}},
	...
	var updatedDocuments []entity.QuotationHistory
	for cursor.Next(ctx) {
		var doc entity.QuotationHistory
		...
		for i := range doc.DadosConsolidados {
			...
			doc.DadosConsolidados[i].Justificativa = []entity.Justification{}
		}
...
func ListPurchaseItems(db *mongodb.MongoDB, quotation int64) ([]entity.ItemPurchase, error) {
...
func UpdatePurchaseItemsJustify(db *mongodb.MongoDB, quotation int64, items *entity.ItemPurchase) error {
	collection := db.Client.Database(schema).Collection("purchases")
...
func DeletePurchaseItems(db *mongodb.MongoDB, quotation int64, items *entity.ItemPurchase) error {
	collection := db.Client.Database(schema).Collection("purchases")
